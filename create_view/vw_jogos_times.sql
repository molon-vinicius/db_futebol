create or alter view vw_jogos_times
...
as  
       select distinct
			...
			, convert(varchar(12)
                     ,case when pen_anf.ID_jogo_time is not null                                          
			...
			, convert(varchar(12)
                     ,case when pen_vis.ID_jogo_time is not null
    ...
    left join (
           select ID_Jogo_Time
             ...
             from tb_jogos_times_penaltis with(nolock)
            group by ID_Jogo_Time
                   , ID_Time
                                   ) pen_anf             on pen_anf.ID_Jogo_Time = src.ID_Jogo_Time
                                                        and pen_anf.ID_Time = src.ID_Time_Anfitriao
    left join (
           select ID_Jogo_Time
                , ID_Time
                , sum(convert(int, Gol)) as Gols
            ...
            group by ID_Jogo_Time
                   , ID_Time
                                   ) pen_vis             on pen_vis.ID_Jogo_Time = src.ID_Jogo_Time
                                                        and pen_vis.ID_Time = src.ID_Time_Visitante														
	...
	left join vw_arbitros               arb4 with(nolock)on arb4.ID_Arbitro = src.ID_arbitro_aux_4 
      group by src.ID_jogo_time 
         ...
         , cf.Descricao
			   , anf.ID_time
			   , anf.Nome_Reduzido
         , convert(varchar(12), case when pen_anf.ID_jogo_time is not null                                          
                                     then '(' + convert(varchar(3), pen_anf.Gols) + ') ' + convert(varchar(3), isnull(a.gols,0))
                                     else convert(varchar(3), isnull(a.gols,0))
                                end )
			   , vis.ID_time
			   , vis.Nome_Reduzido
         , convert(varchar(12), case when pen_vis.ID_jogo_time is not null
                                     then convert(varchar(3), isnull(v.gols,0)) + ' (' + convert(varchar(3), pen_vis.Gols) + ')'
                                     else convert(varchar(3), isnull(v.gols,0))
                                end )
			   , arb.Nome_Reduzido
