...
as
    ...
    left join (
          ...
          , ID_Selecao
            ...
            group by ID_Jogo_Selecao, ID_Selecao
                                   ) pen_anf             on pen_anf.ID_Jogo_Selecao = src.ID_Jogo_Selecao
                                                        and pen_anf.ID_Selecao = src.ID_Selecao_Anfitriao
    left join (
           select ID_Jogo_Selecao
                , ID_Selecao
                , sum(convert(int, Gol)) as Gols
             from tb_jogos_selecoes_penaltis with(nolock)
            group by ID_Jogo_Selecao
                   , ID_Selecao
                                   ) pen_vis             on pen_vis.ID_Jogo_Selecao = src.ID_Jogo_Selecao
                                                        and pen_vis.ID_Selecao = src.ID_Selecao_Visitante
	...
	left join vw_arbitros               arb4 with(nolock)on arb4.ID_Arbitro = src.ID_arbitro_aux_4 
        group by src.ID_jogo_selecao 
               ...
               , dbo.fn_nome_fase(src.ID_Jogo_Selecao)
			   , anf.ID_Selecao
			   , anf.Nome_Selecao
               , convert(varchar(12), case when pen_anf.ID_jogo_selecao is not null                                          
                                           then '(' + convert(varchar(3), pen_anf.Gols) + ') ' + convert(varchar(3), isnull(a.gols,0))
